...
const { orderDetail } = useOrderDetail(route.params.id as string)
</script>
...
<template>
  <div class="order-detail-page" v-if="orderDetail">
    ...
    <OrderMedical :medicines="orderDetail.medicines"></OrderMedical>
    <div class="order-detail">
      <van-cell-group>
        <van-cell title="药品金额" :value="`￥${orderDetail.payment}`" />
        <van-cell title="运费" :value="`￥${orderDetail.expressFee}`" />
        ...
        <van-cell title="支付方式" :value="orderDetail.paymentMethod === 0 ? '微信' : '支付宝'" />
      </van-cell-group>
    </div>
    <!-- 已取消 -->
    <!-- <van-action-bar>
